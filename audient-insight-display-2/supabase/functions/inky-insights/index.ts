...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "full-analysis") {
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ, o assistente de intelig├¬ncia da plataforma MUPA Digital Signage.
Analise os dados do sistema e gere um relat├│rio completo com insights acion├íveis.

Organize sua resposta nas seguintes se├º├Áes com emojis:

## ­ƒÄ» Vis├úo Geral do Sistema
Resumo executivo do estado atual.

## ­ƒæÑ An├ílise de Audi├¬ncia
An├ílise detalhada do p├║blico: g├¬nero, faixa et├íria, emo├º├Áes predominantes, hor├írios de pico.

## ­ƒºá Recomenda├º├Áes Inteligentes
Sugest├Áes de cross-sell, produtos complementares baseados nos dados de consulta de pre├ºo.

## ­ƒÄÑ Segmenta├º├úo Din├ómica de Conte├║do
Sugest├Áes de como segmentar o conte├║do baseado no perfil demogr├ífico, emo├º├Áes e hor├írios.

## ­ƒôê Dashboard de Trade Marketing
An├ílise de audi├¬ncia por campanha/conte├║do, tempo de exposi├º├úo, compara├º├Áes.

## ­ƒÆ░ Monetiza├º├úo de Tela
C├ílculo estimado de valor de exibi├º├úo baseado em audi├¬ncia real, hor├írios e lojas.

## ­ƒñû Insights Autom├íticos
Padr├Áes comportamentais identificados, anomalias, sugest├Áes de melhoria.
...
Formate com markdown rico (tabelas, listas, negrito).`;
      ...
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ. Analise APENAS os dados de audi├¬ncia detalhadamente. Foque em: perfil demogr├ífico, emo├º├Áes, tend├¬ncias, hor├írios de pico, conte├║dos mais engajantes. Use tabelas markdown. Portugu├¬s BR.`;
    } else if (action === "monetization") {
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ. Analise o potencial de monetiza├º├úo das telas. Calcule valores estimados de CPM com base na audi├¬ncia real, hor├írios premium, e valor por loja. Crie uma tabela de precifica├º├úo sugerida. Portugu├¬s BR.`;
    } else if (action === "recommendations") {
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ. Analise os dados de consulta de produtos e recomenda├º├Áes. Sugira estrat├®gias de cross-sell, personaliza├º├úo por perfil, e campanhas baseadas em dados demogr├íficos. Portugu├¬s BR.`;
    } else if (action === "segmentation") {
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ. Crie regras de segmenta├º├úo din├ómica de conte├║do baseadas nos dados de audi├¬ncia. Sugira: qual m├¡dia exibir para qual perfil, hor├írio ideal para cada tipo de conte├║do, prioriza├º├úo de campanhas. Portugu├¬s BR.`;
    } else if (action === "trade-marketing") {
      systemPrompt = `Voc├¬ ├® o Inky ­ƒÉÖ. Gere um relat├│rio de Trade Marketing com: audi├¬ncia por conte├║do, perfil demogr├ífico por m├¡dia exibida, tempo m├®dio de exposi├º├úo, compara├º├úo entre conte├║dos. Use tabelas. Portugu├¬s BR.`;
    }
    ...
    const userContent = question
      ...
      : `Dados do sistema para an├ílise: ${JSON.stringify(systemData)}`;
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return new Response(JSON.stringify({ error: "Limite de requisi├º├Áes excedido." }), {
          status: 429,
