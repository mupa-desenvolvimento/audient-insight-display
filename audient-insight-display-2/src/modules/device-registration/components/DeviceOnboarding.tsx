  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const deviceData = {
        ...
        name: formData.deviceName || `Dispositivo ${deviceId.slice(0, 8)}`,
        store_id: formData.store.id,
        company_id: formData.company.id,
        status: 'online',
        is_active: true,
        store_code: formData.storeNumberOverride || formData.store.code
      };

      let targetDeviceId = existingDevice?.id;
      ...
      // Add to group
      if (targetDeviceId && formData.group.id) {
        await supabase
