...
import { Clock, Shield, Play, Film, AlertCircle } from "lucide-react";
...
export const ResizableChannelBlock = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, edge: "left" | "right") => {
    e.stopPropagation();
    e.preventDefault();
  ...
  const getStatusIcon = () => {
    ...
    if (isLive) return <Play className="w-3.5 h-3.5" />;
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            ref={blockRef}
            className={cn(
              "absolute top-1/2 -translate-y-1/2 h-10 rounded-md border-2 cursor-pointer transition-all flex items-center px-2 gap-1.5 overflow-hidden group",
              colors.bg,
              colors.border,
              colors.text,
          ...
          >
            ...
            {displayStyle.width > 6 && (
              <span className="text-[10px] opacity-80 ml-auto shrink-0 flex items-center gap-0.5 mr-2">
            ...
            {isDragging && (
              <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground text-[10px] font-mono px-2 py-0.5 rounded whitespace-nowrap">
        ...
        <TooltipContent side="top">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground">
              Arraste as bordas para ajustar o horário
            </p>
            {channel.is_fallback && (
